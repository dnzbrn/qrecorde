...
export const events = sqliteTable("events", {
  id: text("id").primaryKey(), ownerId: text("owner_id").notNull().references(() => user.id, { onDelete: "cascade" }), slug: text("slug").notNull(), status: text("status", { enum: ["draft", "published", "archived"] }).notNull().default("draft"), name: text("name").notNull(), eyebrow: text("eyebrow").notNull().default(""), pageTitle: text("page_title").notNull(), pageMessage: text("page_message").notNull().default(""), ctaText: text("cta_text").notNull().default("Abrir meu presente"), giftTitle: text("gift_title").notNull().default("Um presente para você"), giftMessage: text("gift_message").notNull().default(""), accentColor: text("accent_color").notNull().default("#6b29d9"), coverImageKey: text("cover_image_key"), giftImageKey: text("gift_image_key"), instagramImageKey: text("instagram_image_key"), instagramText: text("instagram_text").notNull().default("Eu fui!"), instagramLayout: text("instagram_layout", { enum:["classic","editorial","celebration"] }).notNull().default("editorial"), createdAt: integer("created_at", { mode: "timestamp" }).notNull(), updatedAt: integer("updated_at", { mode: "timestamp" }).notNull(), publishedAt: integer("published_at", { mode: "timestamp" }),
