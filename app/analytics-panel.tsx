...
export function AnalyticsPanel({ eventId, daily, hourly, today, from, to, loaded }: { eventId: string; daily: DailyMetrics[]; hourly: HourlyMetrics[]; today: string; from?: string; to?: string; loaded: boolean }) {
  const rangeFrom = from || analyticsDay(dayStart(today) - 13 * DAY_MS), rangeTo = to || today;
  const [selected, setSelected] = useState(rangeTo);
  const [exporting, setExporting] = useState(false);
  const [note, setNote] = useState('');
  const dayCount = Math.round((dayStart(rangeTo) - dayStart(rangeFrom)) / DAY_MS) + 1;
  const days = Array.from({ length: dayCount }, (_, index) => {
    const day = analyticsDay(dayStart(rangeFrom) + index * DAY_MS);
    return daily.find(item => item.day === day) || { day, ...emptyMetrics() };
  });
  const current = days.find(item => item.day === selected) || days.at(-1)!;
  ...
  const exportReport = async () => {
    setNote('');
    try {
      reportRange(rangeFrom, rangeTo);
      setExporting(true);
      const response = await fetch(`/api/events/${encodeURIComponent(eventId)}/analytics/export?${new URLSearchParams({ from:rangeFrom, to:rangeTo })}`);
      ...
      link.href = url;
      link.download = `analytics-raw-${rangeFrom}-${rangeTo}.csv`;
  ...
  return <section className="panel chart-panel analytics-panel">
    <div className="panel-head"><div><h2>Alcance da página</h2><p>{formatDay(rangeFrom)} a {formatDay(rangeTo)} · Horário de Brasília (UTC−03:00)</p></div></div>
    {!loaded ? <p role="status">Métricas indisponíveis ou carregando. Recarregue a página se persistir.</p> : <>
      <div className="chart-wrap" aria-label="Visualizações por dia">
        ...
        <div className="chart-days"><span>{formatDay(days[0].day).slice(0,5)}</span><span>{formatDay(days[Math.floor((days.length-1)/3)].day).slice(0,5)}</span><span>{formatDay(days[Math.floor((days.length-1)*2/3)].day).slice(0,5)}</span><span>{days.at(-1)?.day===today?'Hoje':formatDay(days.at(-1)!.day).slice(0,5)}</span></div>
      </div>
    ...
    <div className="analytics-export"><h3>Exportar relatório</h3><p>Dados brutos do período selecionado: uma linha por acesso, abertura, download da imagem ou do Story, com dia, horário completo e origem registrada.</p><button className="primary-btn" type="button" disabled={exporting || !eventId} onClick={exportReport}>{exporting ? 'Exportando…' : 'Exportar período em CSV ↓'}</button><p role="status">{note}</p></div>
