...
import { QrExportActions } from "./qr-export-actions";
import { trackedQrUrl } from "../lib/qr-export";
import { analyticsDay, dayStart, DAY_MS, reportRange, type HourlyMetrics } from "../lib/analytics";
...
type AnalyticsData={today:string;from:string;to:string;qrScans:number;hourly:HourlyMetrics[];totals:{views:number;opens:number;downloads:number;stories:number};daily:Array<{day:string;views:number;opens:number;downloads:number;stories:number}>;recent:Array<{id:string;action:string;source:string;createdAt:number}>};
...
function QrCard({ compact = false, url }: { compact?: boolean; url: string }) {
  ...
  useEffect(() => {
    const trackedUrl=trackedQrUrl(url, window.location.origin);
    QRCode.toDataURL(trackedUrl, {
      width: compact ? 180 : 300, margin: 1, color: { dark: "#18151c", light: "#fffdf8" }, errorCorrectionLevel: "H"
    }).then(setSrc);
  }, [compact, url]);
  return src ? <img className="qr-image" src={src} alt="QR Code para abrir a página do presente" /> : <div className="qr-loading" />;
}
  ...
  return (
    <div className="public-shell" style={{"--violet":accentColor} as React.CSSProperties}>
      ...
      <main className="public-hero public-hero-rich">
        ...
        {masterSponsors.length>0&&<aside className={`hero-master-bubbles sponsor-opening ${masterSponsors.length>3?"is-carousel":""}`} aria-label={`${masterSponsors.length} patrocinadores master`}><div className="master-bubble-label"><span>✦</span><small>ESTE PRESENTE É OFERECIDO POR</small><b>Marcas que acreditam neste momento</b><em>{masterSponsors.length} {masterSponsors.length===1?"patrocinador Master":"patrocinadores Master"}</em></div><div className="master-bubble-window"><div className="master-bubble-track">{[0,...(masterSponsors.length>3?[1]:[])].map(copy=><div className="master-bubble-sequence" aria-hidden={copy===1} key={`master-sequence-${copy}`}>{masterSponsors.map((item,i)=><div className="master-bubble" style={{"--bubble-index":i%3} as React.CSSProperties} key={`${item.name}-hero-master-${copy}-${i}`} title={item.name}>{item.media?<><Media asset={item.media} className="master-bubble-logo"/><small className="master-bubble-name">{item.name}</small></>:<strong>{item.name}</strong>}<span>MASTER</span></div>)}</div>)}</div></div><button onClick={() => document.getElementById("apoiadores")?.scrollIntoView({ behavior: "smooth" })}>Ver todos os parceiros <span>↘</span></button></aside>}
...
function Dashboard({ eventId, publicUrl, userName, eventName, analytics, analyticsRange, analyticsLoading, qrScans, onManage, onRangeChange }: { eventId: string; publicUrl: string; userName: string; eventName: string; analytics:AnalyticsData|null; analyticsRange:{from:string;to:string}; analyticsLoading:boolean; qrScans:number; onManage: () => void; onRangeChange:(range:{from:string;to:string})=>void }) {
  const [range, setRange] = useState(analyticsRange); const [rangeError,setRangeError]=useState("");
  const applyRange=()=>{try{reportRange(range.from,range.to);setRangeError("");onRangeChange(range);}catch(error){setRangeError(error instanceof Error?error.message:"Período inválido.");}};
  ...
  return <>
    ...
    <section className="dashboard-period" aria-label="Filtrar métricas por período"><div><b>Período dos resultados</b><small>Todos os indicadores abaixo serão consolidados neste intervalo.</small></div><div className="dashboard-period-fields"><label>De<input type="date" value={range.from} max={range.to} onChange={event=>setRange(current=>({...current,from:event.target.value}))}/></label><label>Até<input type="date" value={range.to} min={range.from} max={analyticsDay()} onChange={event=>setRange(current=>({...current,to:event.target.value}))}/></label><button className="primary-btn" type="button" onClick={applyRange} disabled={analyticsLoading}>{analyticsLoading?"Atualizando…":"Aplicar período"}</button></div>{rangeError&&<p role="alert">{rangeError}</p>}</section>
    ...
    <div className="dashboard-grid">
      <AnalyticsPanel key={`${eventId}-${analyticsRange.from}-${analyticsRange.to}`} eventId={eventId} daily={analytics?.daily||[]} hourly={analytics?.hourly||[]} today={analytics?.today||analyticsDay()} from={analyticsRange.from} to={analyticsRange.to} loaded={!!analytics&&!analyticsLoading} />
      <section className="panel qr-panel"><div className="panel-head"><div><h2>QR Codes</h2><p>Leituras identificadas no período</p></div></div><div className="qr-content"><div className="qr-box"><QrCard compact url={publicUrl}/></div><div className="qr-meta"><span className="live"><i/> Rastreamento ativo</span><b>{qrScans.toLocaleString("pt-BR")}</b><small>leituras no período</small><button onClick={() => navigator.clipboard?.writeText(publicUrl)}>Copiar link público</button></div></div><QrExportActions key={publicUrl} url={publicUrl} eventName={eventName}/></section>
...
function Sponsors({ items, onAdd, onMedia, onChange, onDelete }: { items: SponsorSummary[]; onAdd: (tier?:string) => void; onMedia: (id: string, file: File) => void; onChange: (item: SponsorSummary) => void; onDelete: (id: string) => void }) {
  ...
  return <><div className="welcome compact"><div><span className="kicker">PARCERIAS</span><h1>Patrocinadores</h1><p>Organize até 50 marcas em cada categoria. Master, Ouro e Prata têm destaques proporcionais na página pública.</p></div></div><div className="sponsor-tier-manager">{groups.map(group=>{const groupItems=items.filter(item=>normalized(item.tier)===group.tier);const adding=groupItems.some(item=>item.pending);return <section key={group.tier} className={`manager-tier ${group.tier}`}><header><div><span>{group.label}</span><b>{group.note}</b></div><small>{groupItems.length}/50</small><button className="primary-btn" disabled={groupItems.length>=50||adding} onClick={()=>onAdd(group.tier)}>{adding?"Adicionando…":"Adicionar"} <span>{adding?"·":"＋"}</span></button></header><div className="sponsor-grid">{groupItems.map((item,i)=><article key={item.id} className={item.pending?"is-pending":item.uploading?"is-uploading":""}><div className="sponsor-mark">{item.logoUrl ? <img src={item.logoUrl} alt={`Logo ${item.name}`} className="sponsor-upload-preview"/> : <span>{item.pending?"Preparando…":item.name.split(" ")[0]}</span>}{item.uploading&&<i className="sponsor-uploading">Enviando imagem…</i>}</div><select disabled={item.pending} value={normalized(item.tier)} onChange={e=>onChange({...item,tier:e.target.value})}><option value="master">Master</option><option value="gold">Ouro</option><option value="silver">Prata</option></select><input disabled={item.pending} className="sponsor-name-input" value={item.name} onChange={e=>onChange({...item,name:e.target.value})}/><div><p>Posição na categoria</p><b>#{i+1}</b></div><label className={`upload-inline ${item.pending||item.uploading?"disabled":""}`}>{item.uploading?"Enviando…":item.logoUrl?"Trocar logotipo":"Adicionar logotipo"}<input disabled={item.pending||item.uploading} type="file" accept="image/*" onChange={e => e.target.files?.[0] && onMedia(item.id,e.target.files[0])}/></label><button disabled={item.pending||item.uploading} className="sponsor-delete" onClick={()=>onDelete(item.id)}>Remover</button></article>)}</div>{!groupItems.length&&<p className="tier-empty">Nenhuma marca nesta categoria.</p>}</section>})}</div><div className="panel sponsor-note"><div className="icon">✦</div><div><h2>Marcas em primeiro plano</h2><p>As categorias definem tamanho, ordem e destaque dos logotipos na página do presente.</p></div><span>Visibilidade prioritária</span></div></>;
}
...
export default function Home() {
  ...
  const initialAnalyticsDay = analyticsDay();
  const [analyticsRange,setAnalyticsRange]=useState(()=>({from:analyticsDay(dayStart(initialAnalyticsDay)-13*DAY_MS),to:initialAnalyticsDay}));
  const [analyticsLoading,setAnalyticsLoading]=useState(true);
  const [qrCodes, setQrCodes] = useState<QrSummary[]>([]);
  const sponsorSaveTimers = useRef<Map<string,ReturnType<typeof setTimeout>>>(new Map());
  const title = useMemo(() => nav.find(n => n.id === view)?.label, [view]);
  const activeEvent = events.find(event => event.id === activeEventId);
  const publicUrl = typeof window === "undefined" ? `/${publicSlug}` : `${window.location.origin}/${publicSlug}`;
  ...
  useEffect(() => {
    setAnalytics(null); setQrCodes([]);
    if (!activeEventId) return;
    const controller = new AbortController();
    const get = async (path: string) => { const response = await fetch(path, { signal: controller.signal }); if (!response.ok) throw new Error("Falha ao carregar métricas"); return response.json(); };
    const query=new URLSearchParams(analyticsRange);
    Promise.all([get(`/api/events/${activeEventId}/analytics?${query}`), get(`/api/events/${activeEventId}/qr`)]).then(([metrics, codes]) => {
      if (controller.signal.aborted) return;
      setAnalytics(metrics?.totals ? metrics : null); setQrCodes(Array.isArray(codes) ? codes : []); setAnalyticsLoading(false);
    }).catch(() => { if (!controller.signal.aborted){setAnalytics(null);setAnalyticsLoading(false);} });
    return () => controller.abort();
  }, [activeEventId,analyticsRange]);
  ...
  const addSponsor=async(tier="gold")=>{if(!activeEventId)return;const temporaryId=`pending-${crypto.randomUUID()}`;const optimistic:SponsorSummary={id:temporaryId,name:"Novo patrocinador",tier,tagline:"",pending:true};setSponsors(current=>[...current,optimistic]);try{const response=await fetch(`/api/events/${activeEventId}/sponsors`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:optimistic.name,tier})});const item=await response.json();if(!response.ok)throw new Error(item.error||"Não foi possível adicionar o patrocinador.");setSponsors(current=>current.map(sponsor=>sponsor.id===temporaryId?{...optimistic,id:item.id,tier:item.tier||tier,pending:false}:sponsor));}catch(error){setSponsors(current=>current.filter(sponsor=>sponsor.id!==temporaryId));window.alert(error instanceof Error?error.message:"Não foi possível adicionar o patrocinador.");}};
  const changeSponsor=(changed:SponsorSummary)=>{setSponsors(current=>current.map(item=>item.id===changed.id?changed:item));if(!activeEventId||changed.pending)return;const previous=sponsorSaveTimers.current.get(changed.id);if(previous)clearTimeout(previous);const timer=setTimeout(async()=>{sponsorSaveTimers.current.delete(changed.id);const response=await fetch(`/api/events/${activeEventId}/sponsors/${changed.id}`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify(changed)});if(!response.ok){const result=await response.json();window.alert(result.error||"Não foi possível salvar o patrocinador.");}},450);sponsorSaveTimers.current.set(changed.id,timer);};
  const uploadSponsor=async(id:string,file:File)=>{if(!activeEventId)return;const previous=sponsors.find(item=>item.id===id)?.logoUrl||null;const preview=URL.createObjectURL(file);setSponsors(current=>current.map(item=>item.id===id?{...item,logoUrl:preview,uploading:true}:item));try{const form=new FormData();form.set("file",file);const response=await fetch(`/api/events/${activeEventId}/sponsors/${id}`,{method:"POST",body:form});const result=await response.json();if(!response.ok)throw new Error(result.error||"Falha no upload.");setSponsors(current=>current.map(item=>item.id===id?{...item,logoKey:result.key,logoUrl:result.url,uploading:false}:item));}catch(error){setSponsors(current=>current.map(item=>item.id===id?{...item,logoUrl:previous,uploading:false}:item));window.alert(error instanceof Error?error.message:"Falha no upload.");}finally{URL.revokeObjectURL(preview);}};
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><header className="topbar"><div className="mobile-brand brand"><span className="brand-gem">◆</span><span>QR</span><b>ecorde</b></div><span className="crumb">QRecorde <b>/</b> {title}</span><div className="top-actions"><span className={`top-status ${activeEvent?.status||"draft"}`}>{activeEvent?.status==="published"?"● Publicada":"○ Rascunho"}</span>{activeEvent?.status==="published"&&<a className="top-public-link" href={publicUrl} target="_blank" rel="noreferrer">Abrir link ↗</a>}<button className="publish-btn" onClick={publishEvent}>{activeEvent?.status==="published"?"Atualizar publicação":"Publicar"}</button><button className="preview-btn" onClick={()=>setPublicMode(true)}>Visualizar página <span>↗</span></button></div></header><div className="content-wrap">{view==="dashboard"&&<Dashboard eventId={activeEventId||""} publicUrl={publicUrl} userName={sessionQuery.data.user.name} eventName={pageTitle} analytics={analytics} analyticsRange={analyticsRange} analyticsLoading={analyticsLoading} qrScans={analytics?.qrScans||0} onRangeChange={range=>{setAnalyticsLoading(true);setAnalyticsRange(range);}} onManage={()=>setView("pages")}/>} {view==="pages"&&<PagesManager events={events} activeId={activeEventId} onSelect={selectEvent} onCreate={startNewEvent} onDelete={deleteEvent}/>} {view==="page"&&<PageEditor eventMedia={eventMedia} giftMedia={giftMedia} instagramMedia={instagramMedia} instagramText={instagramText} instagramLayout={instagramLayout} giftTitle={giftTitle} giftMessage={giftMessage} pageEyebrow={pageEyebrow} pageTitle={pageTitle} pageMessage={pageMessage} ctaText={ctaText} publicSlug={publicSlug} publicUrl={publicUrl} isPublished={activeEvent?.status==="published"} accentColor={accentColor} saveState={saveState} onEventMedia={a=>persistMedia("cover",a)} onGiftMedia={a=>persistMedia("gift",a)} onInstagramMedia={a=>persistMedia("instagram",a)} onInstagramText={setInstagramText} onInstagramLayout={setInstagramLayout} onGiftTitle={setGiftTitle} onGiftMessage={setGiftMessage} onPageEyebrow={setPageEyebrow} onPageTitle={setPageTitle} onPageMessage={setPageMessage} onCtaText={setCtaText} onPublicSlug={setPublicSlug} onAccentColor={setAccentColor}/>} {view==="qrcodes"&&<QrDashboard publicUrl={publicUrl} eventName={pageTitle} items={qrCodes} onCreate={createQr} onRename={renameQr} onDelete={removeQr}/>} {view==="sponsors"&&<Sponsors items={sponsors} onAdd={addSponsor} onMedia={uploadSponsor} onChange={changeSponsor} onDelete={deleteSponsor}/>}</div></main>
  </div>;
}
