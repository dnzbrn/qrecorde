...
function Dashboard({ publicUrl, userName, eventName, analytics, qrScans, onManage }: { publicUrl: string; userName: string; eventName: string; analytics:AnalyticsData|null; qrScans:number; onManage: () => void }) {
  ...
  return <>
    ...
    <div className="dashboard-grid">
      ...
      <section className="panel activity-panel"><div className="panel-head"><div><h2>Atividade recente</h2><p>Ações anônimas em tempo real</p></div></div><div className="activity-list">{analytics?.recent.length?analytics.recent.slice(0,6).map(item=><div key={item.id}><span className="avatar purple">{item.action==="view"?"↗":item.action==="open"?"♡":"↓"}</span><p><b>Uma pessoa {actionCopy[item.action]||"interagiu"}</b><small>{ago(item.createdAt)} • {item.source.replace(/^qr_/,"QR: ")}</small></p><strong>{item.action}</strong></div>):<div className="activity-empty"><p><b>Ainda não há atividade</b><small>As interações aparecerão aqui após a publicação.</small></p></div>}</div></section>
    </div>
  </>;
}
...
function QrDashboard({ publicUrl, items, onCreate, onRename, onDelete }: { publicUrl:string;items:QrSummary[];onCreate:()=>void;onRename:(item:QrSummary)=>void;onDelete:(item:QrSummary)=>void }) {
  return <><div className="welcome compact"><div><span className="kicker">CAMPANHAS RASTREÁVEIS</span><h1>Seus QR Codes</h1><p>Crie um código para cada ponto de divulgação e descubra de onde vêm as leituras.</p></div><button className="primary-btn" onClick={onCreate}>Novo QR Code <span>＋</span></button></div><div className="qr-manager-grid">{items.map(item=>{const url=`${publicUrl}?utm_source=qr_${item.campaign}`;return <article className="qr-manager-card" key={item.id}><div className="qr-manager-image"><QrCard compact url={url}/></div><div><span className="live"><i/> Campanha ativa</span><h2>{item.name}</h2><p>{url}</p><strong>{Number(item.scans).toLocaleString("pt-BR")} <small>leituras</small></strong><div className="qr-manager-actions"><button onClick={()=>navigator.clipboard?.writeText(url)}>Copiar link</button><button onClick={()=>onRename(item)}>Editar</button><button className="danger" onClick={()=>onDelete(item)}>Excluir</button></div></div></article>})}</div>{!items.length&&<div className="panel qr-empty"><b>Nenhum QR Code criado</b><p>Crie o primeiro para começar a identificar suas campanhas.</p></div>}</>;
}
...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><header className="topbar"><div className="mobile-brand brand"><span className="brand-gem">◆</span><span>QR</span><b>ecorde</b></div><span className="crumb">QRecorde <b>/</b> {title}</span><div className="top-actions"><button className="publish-btn" onClick={publishEvent}>Publicar</button><button className="preview-btn" onClick={()=>setPublicMode(true)}>Visualizar página <span>↗</span></button></div></header><div className="content-wrap">{view==="dashboard"&&<Dashboard publicUrl={publicUrl} userName={sessionQuery.data.user.name} eventName={pageTitle} analytics={analytics} qrScans={qrCodes.reduce((sum,item)=>sum+Number(item.scans),0)} onManage={()=>setView("pages")}/>} {view==="pages"&&<PagesManager events={events} activeId={activeEventId} onSelect={selectEvent} onCreate={startNewEvent} onDelete={deleteEvent}/>} {view==="page"&&<PageEditor eventMedia={eventMedia} giftMedia={giftMedia} instagramMedia={instagramMedia} instagramText={instagramText} giftTitle={giftTitle} giftMessage={giftMessage} pageEyebrow={pageEyebrow} pageTitle={pageTitle} pageMessage={pageMessage} ctaText={ctaText} accentColor={accentColor} saveState={saveState} onEventMedia={a=>persistMedia("cover",a)} onGiftMedia={a=>persistMedia("gift",a)} onInstagramMedia={a=>persistMedia("instagram",a)} onInstagramText={setInstagramText} onGiftTitle={setGiftTitle} onGiftMessage={setGiftMessage} onPageEyebrow={setPageEyebrow} onPageTitle={setPageTitle} onPageMessage={setPageMessage} onCtaText={setCtaText} onAccentColor={setAccentColor}/>} {view==="qrcodes"&&<QrDashboard publicUrl={publicUrl} items={qrCodes} onCreate={createQr} onRename={renameQr} onDelete={removeQr}/>} {view==="sponsors"&&<Sponsors items={sponsors} onAdd={addSponsor} onMedia={uploadSponsor} onChange={changeSponsor} onDelete={deleteSponsor}/>}</div></main>
  </div>;
}
