...
import { analyticsDay, type HourlyMetrics } from "../lib/analytics";
...
type AnalyticsData={today:string;hourly:HourlyMetrics[];totals:{views:number;opens:number;downloads:number;stories:number};daily:Array<{day:string;views:number;opens:number;downloads:number;stories:number}>;recent:Array<{id:string;action:string;source:string;createdAt:number}>};
...
function Dashboard({ eventId, publicUrl, userName, eventName, analytics, qrScans, onManage }: { eventId: string; publicUrl: string; userName: string; eventName: string; analytics:AnalyticsData|null; qrScans:number; onManage: () => void }) {
  ...
  return <>
    ...
    <div className="dashboard-grid">
      <AnalyticsPanel key={eventId} eventId={eventId} daily={analytics?.daily||[]} hourly={analytics?.hourly||[]} today={analytics?.today||analyticsDay()} loaded={!!analytics} />
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([get(`/api/events/${activeEventId}/analytics`), get(`/api/events/${activeEventId}/qr`)]).then(([metrics, codes]) => {
      if (controller.signal.aborted) return;
      setAnalytics(metrics?.totals ? metrics : null); setQrCodes(Array.isArray(codes) ? codes : []);
    }).catch(() => { if (!controller.signal.aborted) setAnalytics(null); });
