...
export function PublicPage({ eventMedia, giftMedia, sponsorMedia, sponsorNames = ["Vallourec", "Unimed BH", "Supermercados BH"], giftTitle, giftMessage, pageEyebrow, pageTitle, pageMessage, ctaText, trackingSlug, accentColor = "#6b29d9" }: { eventMedia: MediaAsset; giftMedia: MediaAsset; sponsorMedia: (MediaAsset | null)[]; sponsorNames?: string[]; giftTitle: string; giftMessage: string; pageEyebrow: string; pageTitle: string; pageMessage: string; ctaText: string; trackingSlug?: string; accentColor?:string }) {
...
export default function Home() {
  ...
  useEffect(()=>{if(!activeEventId)return;setEventHydrated(false);fetch(`/api/events/${activeEventId}`).then(r=>r.json()).then(data=>{setPageTitle(data.pageTitle||data.name);setPageEyebrow(data.eyebrow||"");setPageMessage(data.pageMessage||"");setCtaText(data.ctaText||"Abrir meu presente");setGiftTitle(data.giftTitle||"Um presente para você");setGiftMessage(data.giftMessage||"");setAccentColor(data.accentColor||"#6b29d9");setEventMedia(data.coverImageKey?{src:`/media/${data.coverImageKey}`,type:"image",name:"Imagem da página"}:{src:"/padre-eustaquio-2026.jpg",type:"image",name:"Imagem de exemplo"});setGiftMedia(data.giftImageKey?{src:`/media/${data.giftImageKey}`,type:"image",name:"Imagem do presente"}:{src:"/presente-padre-eustaquio.png",type:"image",name:"Presente de exemplo"});const loadedSponsors=(data.sponsors||[]).map((item:SponsorSummary)=>({...item,logoUrl:item.logoKey?`/media/${item.logoKey}`:null}));setSponsors(loadedSponsors);setEventHydrated(true);});},[activeEventId]);
  useEffect(()=>{if(!activeEventId||!eventHydrated)return;setSaveState("Salvando...");const timer=setTimeout(()=>fetch(`/api/events/${activeEventId}`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({name:pageTitle,pageTitle,pageMessage,eyebrow:pageEyebrow,ctaText,giftTitle,giftMessage,accentColor})}).then(()=>{setSaveState("Salvo");setEvents(current=>current.map(item=>item.id===activeEventId?{...item,name:pageTitle,pageTitle}:item));}),650);return()=>clearTimeout(timer);},[activeEventId,eventHydrated,pageTitle,pageMessage,pageEyebrow,ctaText,giftTitle,giftMessage,accentColor]);
  ...
  if (publicMode) return <div><button className="back-dashboard" onClick={() => setPublicMode(false)}>← Voltar ao painel</button><PublicPage eventMedia={eventMedia} giftMedia={giftMedia} sponsorMedia={sponsors.map(item=>item.logoUrl?{src:item.logoUrl,type:"image",name:item.name}:null)} sponsorNames={sponsors.map(item=>item.name)} giftTitle={giftTitle} giftMessage={giftMessage} pageEyebrow={pageEyebrow} pageTitle={pageTitle} pageMessage={pageMessage} ctaText={ctaText} accentColor={accentColor}/></div>;
