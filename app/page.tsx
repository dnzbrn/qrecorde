...
export function PublicPage({ eventMedia, giftMedia, instagramMedia, instagramText, instagramLayout = "editorial", sponsorMedia, sponsorNames = ["Vallourec", "Unimed BH", "Supermercados BH"], giftTitle, giftMessage, pageEyebrow, pageTitle, pageMessage, ctaText, trackingSlug, accentColor = "#6b29d9" }: { eventMedia: MediaAsset; giftMedia: MediaAsset; instagramMedia:MediaAsset; instagramText:string; instagramLayout?:InstagramLayout; sponsorMedia: (MediaAsset | null)[]; sponsorNames?: string[]; giftTitle: string; giftMessage: string; pageEyebrow: string; pageTitle: string; pageMessage: string; ctaText: string; trackingSlug?: string; accentColor?:string }) {
  ...
  const drawInstagramLayout = (ctx:CanvasRenderingContext2D, image:HTMLImageElement) => {
    const title=pageTitle.trim().slice(0,100);ctx.save();ctx.shadowBlur=0;
    if(instagramLayout==="classic"){
      const ratio=Math.min(700/image.width,760/image.height);const w=image.width*ratio,h=image.height*ratio;ctx.drawImage(image,(1080-w)/2,55,w,h);const copy=storyLines(ctx,instagramText,760,64);const y=Math.max(850,80+h);ctx.fillStyle=accentColor;ctx.beginPath();ctx.roundRect(110,y,860,copy.lines.length===1?120:170,60);ctx.fill();ctx.fillStyle="#fff";ctx.textAlign="center";ctx.textBaseline="middle";ctx.font=`800 ${copy.size}px Arial`;copy.lines.forEach((line,i)=>ctx.fillText(line,540,y+(copy.lines.length===1?60:55+i*(copy.size+8)),760));ctx.fillStyle="#281d2d";ctx.font="italic 700 34px Georgia";ctx.fillText(title,540,y+(copy.lines.length===1?175:225),820);
    }else if(instagramLayout==="celebration"){
      ctx.strokeStyle=accentColor;ctx.lineWidth=5;ctx.beginPath();ctx.arc(540,420,350,Math.PI,0);ctx.lineTo(890,760);ctx.stroke();const ratio=Math.min(650/image.width,720/image.height);const w=image.width*ratio,h=image.height*ratio;ctx.drawImage(image,(1080-w)/2,80,w,h);for(const side of[-1,1])for(let i=0;i<5;i++){ctx.save();ctx.translate(540+side*(350+i*18),520+i*65);ctx.rotate(side*(.45+i*.04));ctx.fillStyle=i%2?accentColor:"#d2a64b";ctx.beginPath();ctx.ellipse(0,0,12,34,0,0,Math.PI*2);ctx.fill();ctx.restore();}const copy=storyLines(ctx,instagramText,760,64);ctx.fillStyle="#281d2d";ctx.textAlign="center";ctx.textBaseline="middle";ctx.font=`italic 800 ${copy.size}px Georgia`;copy.lines.forEach((line,i)=>ctx.fillText(line,540,920+i*(copy.size+8),760));ctx.fillStyle=accentColor;ctx.fillRect(380,1025,320,5);ctx.font="700 30px Arial";ctx.fillText(title.toUpperCase(),540,1080,800);
    }else{
      ctx.strokeStyle=accentColor;ctx.lineWidth=4;ctx.beginPath();ctx.arc(830,245,190,0,Math.PI*2);ctx.stroke();ctx.fillStyle=accentColor;ctx.fillRect(78,75,12,760);const ratio=Math.min(670/image.width,750/image.height);const w=image.width*ratio,h=image.height*ratio;ctx.drawImage(image,125,70,w,h);const copy=storyLines(ctx,instagramText,800,76);ctx.fillStyle="#281d2d";ctx.textAlign="left";ctx.textBaseline="alphabetic";ctx.font=`800 ${copy.size}px Arial`;copy.lines.forEach((line,i)=>ctx.fillText(line,120,900+i*(copy.size+5),800));const ruleY=copy.lines.length===1?945:1005;ctx.fillStyle=accentColor;ctx.fillRect(120,ruleY,160,8);ctx.font="italic 700 37px Georgia";drawWrappedText(ctx,title,120,ruleY+65,780,45,2);
    }
    ctx.textAlign="center";ctx.fillStyle=accentColor;ctx.font="700 18px Arial";ctx.fillText("QRECORDE  ✦  PRESENTE DIGITAL",540,1300);ctx.restore();
  };
...
export default function Home() {
  ...
  if (publicMode) return <div><button className="back-dashboard" onClick={() => setPublicMode(false)}>← Voltar ao painel</button><PublicPage eventMedia={eventMedia} giftMedia={giftMedia} instagramMedia={instagramMedia} instagramText={instagramText} instagramLayout={instagramLayout} sponsorMedia={sponsors.map(item=>item.logoUrl?{src:item.logoUrl,type:"image",name:item.name}:null)} sponsorNames={sponsors.map(item=>item.name)} giftTitle={giftTitle} giftMessage={giftMessage} pageEyebrow={pageEyebrow} pageTitle={pageTitle} pageMessage={pageMessage} ctaText={ctaText} accentColor={accentColor}/></div>;
