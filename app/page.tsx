...
export function PublicPage({ eventMedia, giftMedia, instagramMedia, instagramText, instagramLayout = "editorial", sponsorMedia, sponsorNames = ["Vallourec", "Unimed BH", "Supermercados BH"], giftTitle, giftMessage, pageEyebrow, pageTitle, pageMessage, ctaText, trackingSlug, accentColor = "#6b29d9" }: { eventMedia: MediaAsset; giftMedia: MediaAsset; instagramMedia:MediaAsset; instagramText:string; instagramLayout?:InstagramLayout; sponsorMedia: (MediaAsset | null)[]; sponsorNames?: string[]; giftTitle: string; giftMessage: string; pageEyebrow: string; pageTitle: string; pageMessage: string; ctaText: string; trackingSlug?: string; accentColor?:string }) {
  ...
  const drawInstagramLayout = (ctx:CanvasRenderingContext2D, image:HTMLImageElement) => {
    ...
    if(instagramLayout==="classic"){
      ...
      const ratio=Math.min(670/image.width,620/image.height);const w=image.width*ratio,h=image.height*ratio;ctx.drawImage(image,(1080-w)/2,20,w,h);const copy=storyLines(ctx,instagramText,800,76);const copyY=Math.max(675,40+h);ctx.fillStyle="#281d2d";ctx.textAlign="left";ctx.textBaseline="alphabetic";ctx.font=`800 ${copy.size}px Arial`;copy.lines.forEach((line,i)=>outlined(line,120,copyY+i*(copy.size+5),800));const ruleY=copyY+(copy.lines.length-1)*(copy.size+5)+42;ctx.fillStyle=accentColor;ctx.fillRect(120,ruleY,160,8);ctx.fillStyle=accentColor;ctx.font="italic 700 37px Georgia";const titleWords=title.split(/\s+/);let line="";let lineIndex=0;for(const word of titleWords){const candidate=line?`${line} ${word}`:word;if(ctx.measureText(candidate).width>780&&line){outlined(line,120,ruleY+62+lineIndex*45,780);line=word;lineIndex++;}else line=candidate;}if(line)outlined(line,120,ruleY+62+lineIndex*45,780);
    }
